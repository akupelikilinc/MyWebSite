...
document.addEventListener('DOMContentLoaded', function() {
    ...
    tabButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            // YouTube tab'ına tıklandığında videoları yükle
            if (tabId === 'youtube') {
                const youtubeGrid = document.getElementById('youtubeGrid');
                if (youtubeGrid && !youtubeGrid.children.length) {
                    fetchYouTubeVideos();
                }
            }
        });
...
async function fetchYouTubeVideos() {
    ...
    try {
        ...
        if (!channelResponse.ok) {
            throw new Error(`Kanal bilgileri alınamadı: ${channelResponse.status}`);
        }
        ...
        // Uploads playlist'inden videoları al
        const videosResponse = await fetch(`https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId=${uploadsPlaylistId}&maxResults=${MAX_RESULTS}&key=${YOUTUBE_API_KEY}`);
        
        if (!videosResponse.ok) {
            throw new Error(`Videolar alınamadı: ${videosResponse.status}`);
        }
...
function displayVideos(data) {
    ...
    if (data.items && data.items.length > 0) {
        data.items.forEach(item => {
            ...
            // Playlist item'larında video ID snippet.resourceId.videoId'de bulunur
            ...
            videoCard.innerHTML = `
                <div class="video-thumbnail">
                    <a href="https://www.youtube.com/watch?v=${videoId}" target="_blank" rel="noopener noreferrer">
                        <img src="${snippet.thumbnails ? snippet.thumbnails.maxres?.url || snippet.thumbnails.high?.url || snippet.thumbnails.medium?.url : `https://img.youtube.com/vi/${videoId}/maxresdefault.jpg`}" alt="${snippet.title}">
...
// YouTube videoları artık tab'a tıklandığında yüklenecek